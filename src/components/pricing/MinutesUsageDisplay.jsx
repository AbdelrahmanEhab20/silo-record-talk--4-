...
export default function MinutesUsageDisplay({ used, total, label, isDaily }) {
  ...
  const color = isExhausted ? '#EF4444' : isLow ? '#F59E0B' : '#A855F7';
  ...
  return (
    <div className="rounded-2xl p-4" style={{ backgroundColor: isDark ? '#1C1C1E' : '#F5F5F7' }}>
      ...
      <div className="w-full h-2 rounded-full overflow-hidden mb-1" style={{ backgroundColor: isDark ? '#404040' : '#E8E8ED' }}>
        <motion.div
          initial={{ width: 0 }}
      ...
      {isExhausted && (
        <p className="text-xs mt-2" style={{ color: '#EF4444' }}>
          🔴 Limit reached. Watch an ad or upgrade to Pro.
        </p>
      )}
    </div>
  );
